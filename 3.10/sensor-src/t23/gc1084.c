 ...
 *   0         1280*720       30       mipi_1lane      linear
 */
...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	hb = (hb << 8) + tmp;
	ret += sensor_read(sd, 0x000f, &tmp);
	win_width = tmp;
	ret += sensor_read(sd, 0x0010, &tmp);
	win_width = (win_width << 8) + tmp;
	if (ret < 0)
		return -1;
	hts = hb;
	ret = sensor_read(sd, 0x0d0d, &tmp);
	win_high = tmp;
	ret += sensor_read(sd, 0x0d0e, &tmp);
	if (ret < 0)
		return -1;
	win_high = (win_high << 8) + tmp;
	vts = pclk * (fps & 0xffff) / hts / ((fps & 0xffff0000) >> 16);
	vb = vts - win_high - 16;
	ret = sensor_write(sd, 0x0d42, (unsigned char)(vb & 0xff));
	ret += sensor_write(sd, 0x0d41, (unsigned char)(vb >> 8));
	if (ret < 0)
		return -1;
...
static int sensor_g_chip_ident(struct tx_isp_subdev *sd, struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("%s chip found @ 0x02x (%s)\n",
